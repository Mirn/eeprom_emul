...
3. Дополнительно сохраняется номер переменной проинвертированный.  8 бит = 1 байт. Это нужно чтоб чтобы можно было различить не записанную запись от записанной, даже если переменная, её адрес и crc все биты равны единицам. Это гарантирует что в записи будут нулевые биты.
4. КРК16 код 16 бит = 2 байта - нужен чтоб отличить мусор и повторную перезапись в случае сбоев. А так-же недоконца стёртых страниц - в них бывает мусор.
