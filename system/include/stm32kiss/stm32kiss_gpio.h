...
typedef struct
{
	...
	void (*RCC_PeriphClockCmd)(uint32_t RCC_APBPeriph, FunctionalState NewState);
...
#define pin_mode_analog_input   GPIO_Mode_AIN
#define pin_mode_analog_output  GPIO_Mode_AF_PP
#define pin_mode_analog_drain   GPIO_Mode_AF_OD
#define pin_mode_input          GPIO_Mode_IN_FLOATING
#define pin_mode_input_up       GPIO_Mode_IPU
#define pin_mode_input_down     GPIO_Mode_IPD
#define pin_mode_output         GPIO_Mode_Out_PP
#define pin_mode_open_drain     GPIO_Mode_Out_OD

void pin_mode(const KS_PIN *pin, GPIOMode_TypeDef pin_mode);

#define pin_analog_input(pin)  pin_mode(pin, pin_mode_analog_input)
#define pin_analog_output(pin) pin_mode(pin, pin_mode_analog_output)
#define pin_analog_drain(pin)  pin_mode(pin, pin_mode_analog_drain)
#define pin_input(pin)	    pin_mode(pin, pin_mode_input)
#define pin_input_up(pin)   pin_mode(pin, pin_mode_input_up)
#define pin_input_down(pin) pin_mode(pin, pin_mode_input_down)
#define pin_output(pin)     pin_mode(pin, pin_mode_output)
#define pin_open_drain(pin) pin_mode(pin, pin_mode_open_drain)
...
static inline void pin_output_v(const KS_PIN *pin, bool new_value)
{
	pin_mode(pin, pin_mode_output);
	pin_write(pin, new_value);
}
...
static __inline__ void ___bind_mode_n(const KS_PIN **bind, uint32_t mode, uint8_t count)
{
	uint32_t k;

	for (k=0; k < count; k++)
		pin_mode(bind[k], mode);
}
...
static __inline__ void fast_pin_mode(const KS_PIN *pin, uint32_t mode)
{
...
#define FAST_MODE_WRITE       0x03
#define FAST_MODE_DRAIN       0x07
#define FAST_MODE_WRITE_ALT   0x0B
#define FAST_MODE_DRAIN_ALT   0x0F
#define FAST_MODE_READ_ANALOG 0x00
#define FAST_MODE_READ        0x04
#define FAST_MODE_READ_PULL   0x08

static __inline__ void fast_pin_input(const KS_PIN *pin) {fast_pin_mode(pin, FAST_MODE_READ);};
static __inline__ void fast_pin_input_up(const KS_PIN *pin)   {pin->GPIO->BSRR = pin->mask; fast_pin_mode(pin, FAST_MODE_READ_PULL);};
static __inline__ void fast_pin_input_down(const KS_PIN *pin) {pin->GPIO->BRR  = pin->mask; fast_pin_mode(pin, FAST_MODE_READ_PULL);};
static __inline__ void fast_pin_output_v(const KS_PIN *pin, bool v) {fast_pin_mode(pin, FAST_MODE_WRITE); pin_write(pin, v);};
