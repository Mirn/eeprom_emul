...
int32_t fifo_count(tFIFO *fifo)
{
	...
	if (cnt > (int32_t)fifo->size) return fifo->size;
	if (cnt < 0) return 0;
...
void fifo_check_read(tFIFO *fifo)
{
	...
	if ((int32_t)fifo->stat_min > cnt) fifo->stat_min = cnt;
	if ((int32_t)fifo->stat_max < cnt) fifo->stat_max = cnt;
